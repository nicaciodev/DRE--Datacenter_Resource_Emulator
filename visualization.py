...
# Importa as classes do nosso modelo para usar nos type hints
from datacenter_model import ServidorFisico, MaquinaVirtual

# Tenta importar o Matplotlib, mas não quebra o programa se não estiver instalado.
try:
    import matplotlib
...
# ALTERAÇÃO AQUI: Adicionado o parâmetro 'plot_x_offset'
def draw_datacenter_state(screen: pygame.Surface, font: pygame.font.Font, servidores: List[ServidorFisico], best_solution: List[int], vms: List[MaquinaVirtual], plot_x_offset: int):
    ...
    """
    # Cria uma cópia temporária dos servidores para simular a alocação
    temp_servidores = [ServidorFisico(s.id, s.cpu_total, s.ram_total) for s in servidores]

    # Aloca as VMs nos servidores temporários de acordo com a solução
    ...
    # Define o layout inicial para desenhar os servidores
    x, y = PADDING, PADDING
    # A variável agora vem do parâmetro da função
    max_x = screen.get_width() - SERVER_WIDTH - (plot_x_offset if MATPLOTLIB_AVAILABLE else 0)

    # Itera sobre cada servidor para desenhá-lo
    for servidor in temp_servidores:
        draw_servidor(screen, font, servidor, x, y)
        
        # Move a posição para o próximo servidor
